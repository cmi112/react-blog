import React from 'react'
import 'bootstrap/dist/css/bootstrap.min.css';
import { Row, Col,Container} from "react-bootstrap"




export default function Home() {
    return (
        <>
       
        <div className="hero">
        <h1 style={{margin:"80px",color:"greenyellow"}}>I am Abdilkani </h1>
        <div className="hero-info">
        <div className="left-text-hero">
        <h3>Web Programer Based in Wetzlar Germany</h3>
      <p>Lorem ipsum dolor sit, amet consectetur adipisicing elit. Expedita aspernatur, mollitia repellendus provident, eligendi animi hic blanditiis quos non odit rerum odio! Corrupti aspernatur a ipsum deleniti quia provident nulla.</p>
        </div>
       <div className="right-hero-img">
         <img className="profile" src="data:image/jpeg;base64," alt=""/>
         

        </div>
      
          
       </div>
          

        </div>
          
    
        <Container >
          <h3>My Skills</h3>
  <Row>
    <Col>    <section className="html-sec" >
        
           <img src="https://techbeacon.com/sites/default/files/styles/social/public/html5-mobile-app-native-hybrid-pros-cons.jpg?itok=R3pMKIzh" alt=""/>
           <h2 style={{margin:"80px",color:"rgba(93, 185, 210, 0.681)"}}>HTML5 </h2>
            <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Quasi minima error porro nulla ab. Hic officiis, fugit, pariatur facere aliquid quaerat dolores quibusdam ullam atque accusamus ratione rerum doloribus nihil?</p>
            <a href="https://google.com"><button>Read More....</button></a>
           </section></Col>
    <Col>
    <section className="css-sec">
   
           <img src="https://colorlib.com/wp/wp-content/uploads/sites/2/creative-css3-tutorials.jpg" alt=""/>
           <h2 style={{margin:"80px",color:"rgba(93, 185, 210, 0.681)"}}>CSS3 </h2>
           <h3>CSS Frameworks</h3>
           <ol>
             <li>Bootstrap</li>
             <li>Sass</li>
           </ol>
            <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Quasi minima error porro nulla ab. Hic officiis, fugit, pariatur facere aliquid quaerat dolores quibusdam ullam atque accusamus ratione rerum doloribus nihil?</p>
            <a href="https://google.com"><button>Read More....</button></a>
           </section>
    </Col>
  </Row>
  <Row>
    <Col>  <section className="js-sec">
    
                <img src="https://www.codingame.com/servlet/fileservlet?id=15562579750770" alt=""/>
                <h2 style={{margin:"80px",color:"rgba(93, 185, 210, 0.681)"}}>JavaScript </h2>
                <h3>JS Frameworks and Libereries</h3>
           <ol>
             <li>Jquery</li>
             <li>React</li>
           </ol>
                <p>Lorem ipsum dolor, sit amet consectetur adipisicing elit. Laudantium fugit vel quaerat perferendis sit et adipisci placeat. Culpa dolor eos illum iusto rerum totam repellat! Adipisci, totam enim? Placeat, sit.</p>
                <a href="https://google.com"><button>Read More....</button></a>
            </section></Col>
    <Col>  <section>

                <img src="https://miro.medium.com/max/1838/1*xdo0UBpyszvD7-7EH4TkIA.png" alt=""/>
                <h2 style={{margin:"80px",color:"rgba(93, 185, 210, 0.681)"}}>Nodejs </h2>
                <p>Lorem ipsum dolor, sit amet consectetur adipisicing elit. Laudantium fugit vel quaerat perferendis sit et adipisci placeat. Culpa dolor eos illum iusto rerum totam repellat! Adipisci, totam enim? Placeat, sit.</p>
                <a href="https://google.com"><button>Read More....</button></a>
            </section></Col>
    <Col>  <section>
   
                <img src="https://cdn.worldvectorlogo.com/logos/mongodb.svg" alt=""/>
                <h2 style={{margin:"80px",color:"rgba(93, 185, 210, 0.681)"}}>MongoDB </h2>
                <p>Lorem ipsum dolor, sit amet consectetur adipisicing elit. Laudantium fugit vel quaerat perferendis sit et adipisci placeat. Culpa dolor eos illum iusto rerum totam repellat! Adipisci, totam enim? Placeat, sit.</p>
                <a href="https://google.com"><button>Read More....</button></a>
            </section></Col>
            
            <Col>  <section>
   <img src="https://buddy.works/guides/covers/first-steps-with-git/first-steps-git-cover.png" alt=""/>
   <h2 style={{margin:"80px",color:"rgba(93, 185, 210, 0.681)"}}>Git and Version Control</h2>
   <p>Lorem ipsum dolor, sit amet consectetur adipisicing elit. Laudantium fugit vel quaerat perferendis sit et adipisci placeat. Culpa dolor eos illum iusto rerum totam repellat! Adipisci, totam enim? Placeat, sit.</p>
   <a href="https://google.com"><button>Read More....</button></a>
</section></Col>
<Col>  <section className="sql-sec">
   <img src="https://i0.wp.com/learn.onemonth.com/wp-content/uploads/2019/07/image2-1.png?fit=600%2C315&ssl=1" alt=""/>
   <h2 style={{margin:"80px",color:"rgba(93, 185, 210, 0.681)"}}>SQL</h2>
   <p>Lorem ipsum dolor, sit amet consectetur adipisicing elit. Laudantium fugit vel quaerat perferendis sit et adipisci placeat. Culpa dolor eos illum iusto rerum totam repellat! Adipisci, totam enim? Placeat, sit.</p>
   <a href="https://google.com"><button>Read More....</button></a>
</section></Col>
  </Row>
</Container>

 
       
        </>
    )
}
